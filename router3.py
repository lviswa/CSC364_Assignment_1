...
def processing_thread(connection, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    while True:
        ...
        # 5(c): send or out or discard
        if not is_last_hop and new_ttl > 0 and send_interface == "d":
...
def start_server():
    ...
    while True:
        ...
        try:
            Thread(target=processing_thread, args=(connection, forwarding_table_with_range, default_gateway_port)).start()
        except:
