...
# find default port when no match in forwarding table
...
# generate forwarding table with ip range for each interface
...
# convert string ip to binary representation
def ip_to_bin(ip):
    ip_octets = ip.split(".")
    ip_bin_string = ""
...
# find range of ips for destination ip/subnet mask pair
...
# receive packets and forward or discard
def processing_thread(connection, forwarding_table_with_range, default_gateway_port,
                     conn_b, conn_c, sock_8005, sock_8006, max_buffer_size=5120):
    while True:
...
# create server socket, listen, accept two connections (c then b), spawn thread per connection
def start_server():
    host = "127.0.0.1"
    port = 8004
    soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    sock_8005 = create_socket(host, 8005)
    sock_8006 = create_socket(host, 8006)

    conn_c = soc.accept()[0]
    print("First connection accepted (c - R2)")
    conn_b = soc.accept()[0]
    print("Second connection accepted (b - R1)")

    for conn in [conn_b, conn_c]:
        try:
            Thread(target=processing_thread, args=(conn, forwarding_table_with_range, default_gateway_port, conn_b, conn_c, sock_8005, sock_8006)).start()
        except:
